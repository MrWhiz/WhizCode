...
const SKIP_DIRS = /* @__PURE__ */ new Set(["node_modules", ".git", "dist", "dist-electron", ".next", "__pycache__", ".venv", "venv", ".cache", "coverage", ".idea", ".vscode"]);
const BINARY_EXTS = /* @__PURE__ */ new Set([".png", ".jpg", ".jpeg", ".gif", ".ico", ".webp", ".svg", ".woff", ".woff2", ".ttf", ".eot", ".mp3", ".mp4", ".zip", ".tar", ".gz", ".exe", ".dll", ".so", ".dylib", ".lock", ".pdf"]);
...
async function runAgentLoop(userMessage, provider, config, workspacePath) {
  const steps = [];
  if (workspacePath && !workspaceContextLoaded) {
    ...
    const files = await readDirectoryRecursive(workspacePath, 200);
    if (files.length > 0) {
      let manifest = `
...
`;
      ...
      for (const f of files) {
        ...
        if (importantExts.includes(ext || "") || f.content.length < 5e3) {
          manifest += `
--- FILE: ${rel} ---
${f.content}
`;
