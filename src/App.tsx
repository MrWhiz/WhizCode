...
import SkillsPanel from "./components/Explorer/SkillsPanel";
...
function App() {
    ...
    (action: string) => {
      ...
      switch (action) {
                ...
                setMessages([
                  {
                    role: "assistant",
                    content:
                      "Hello! I'm your WhizCode agent. Open a folder to get started.",
                  },
                ]);
                setAgentSteps([]);
                setLiveStreamingContent("");
        ...
        case "toggle-source-control":
          setActiveView(
            activeView === "source-control" ? null : "source-control",
          );
          break;
  ...
  const refreshPlanningArtifacts = useCallback(
    async (
      workspacePathArg?: string | null,
      ...
      try {
        ...
        const selectedSpec =
          ...
          (currentPlan
            ? specList.find((item: any) => item.id === currentPlan.id)
            : null) ||
          specList[0];
  ...
  React.useEffect(() => {
    ...
    const loadWorkspaceHistory = async () => {
      ...
      try {
        ...
        console.debug(
          "No existing workspace history to restore:",
          workspacePath,
          error,
        );
      } finally {
  ...
  React.useEffect(() => {
    ...
    const setupPlanningListeners = async () => {
      ...
      unlistenTaskSnapshot = await agent.events.onTaskSnapshotUpdated(
        (snapshot) => {
          setTaskSnapshot(snapshot);
        },
      );
  ...
  const buildAgentModelConfig = useCallback(
    () => ({
      ...
      azureUsername,
      azureSessionToken:
        azureSessionToken.trim() && azureTokenExpiresAt > Date.now()
          ? azureSessionToken
          : "",
    }),
    [
  ...
  const handleSend = async (overrideInput?: string) => {
    ...
    if ((!textToSend.trim() && selectedImages.length === 0) || isLoading)
      return;
    ...
    try {
      if (!workspacePath) {
        setMessages((prev) => [
          ...prev,
          {
            role: "assistant",
            content:
              "⚠️ No workspace open. Please open a folder first (File → Open Folder) to use the agent.",
          },
        ]);
        setIsLoading(false);
        return;
      ...
      const finalSteps = collectResultSteps(
        result,
        latestAgentStepsRef.current,
      );
      ...
      let finalResponse = appendVerificationSummary(
        response,
        verificationSteps,
      );
  ...
  const handleAskUserResponse = async (response: string) => {
    try {
      ...
      await agent.sendAskUserResponse(
        trimmedResponse,
        askUserPrompt.requestId || undefined,
      );
    } catch (error) {
        ...
        {isChatOpen && (
          <ChatPanel
            ...
            settingsProps={{
              ...
              setAzurePassword,
              azureTokenStatus,
              onGenerateAzureToken: handleGenerateAzureToken,
              isAutopilotMode,
              setIsAutopilotMode,
              contextLength,
              setContextLength,
...
function buildPersistedMessages(
  ...
  const baseMessages = messages.filter(
    (message) => !(message as any).__whizcodeDraft,
  );
  ...
  const hasLiveTaskState =
    agentSteps.length > 0 || liveStreamingContent.trim().length > 0;
  if (!hasLiveTaskState) {
    return baseMessages;
  }
  ...
  const draftMessage: Message & {
    ...
    content:
      ...
      "Task was in progress when the app closed. The latest live logs are preserved below.",
...
function collectResultSteps(result: any, liveSteps: AgentStep[]): AgentStep[] {
  ...
  return liveSteps.length > 0 ? [...liveSteps] : [...toolSteps, ...steps];
}
...
function resolveAgentResponse(result: any, steps: AgentStep[]): string {
  ...
  const failedStep = [...steps]
    .reverse()
    .find((step) => step.status === "failed" || step.status === "error");
...
function getFailedReviewStep(
  verificationSteps: AgentStep[],
): AgentStep | undefined {
  return verificationSteps.find(
    (step) => step.tool === "review" && step.status === "failed",
  );
}
...
function remapRepairSteps(
  ...
  return steps.map((step, index) => ({
    ...
    requestId: step.requestId
      ? `repair-${step.requestId}`
      : `repair-step-${iterationOffset + index + 1}`,
  }));
}

async function buildVerificationSteps(
  ...
  try {
    ...
    const findingsText =
      report.findings.length > 0
        ? report.findings
            ...
            .map(
              (finding) =>
                `${finding.severity.toUpperCase()} ${finding.file}:${finding.line} ${finding.message}${finding.suggestion ? ` (${finding.suggestion})` : ""}`,
            )
    ...
    const recoveryLogs = [
      ...
      ...(recovery.suggested_action
        ? [`Suggested action: ${recovery.suggested_action}`]
        : []),
...
function appendVerificationSummary(
  response: string,
  verificationSteps: AgentStep[],
): string {
...
async function runAutomaticRepairPass(params: {
  ...
  setAgentSteps: (
    steps: AgentStep[] | ((prev: AgentStep[]) => AgentStep[]),
  ) => void;
  ...
  try {
    ...
    const repairResponse =
      repairResult?.response || "Automatic repair pass completed.";
    ...
    const combinedSteps = [
      ...params.initialSteps,
      ...params.initialVerificationSteps,
      ...repairSteps,
    ];
    const reverifiedSteps = await buildVerificationSteps(
